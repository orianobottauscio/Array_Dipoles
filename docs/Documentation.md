...
The local coordinate system {*u*,*v*,*w*} is always located with axis *w* along the *z* axis of the global coordinate system. The magnet can be rotated in the *xy*-plane by an angle $\alpha$. The geometrical data describing the PMs are detailed in section PM Input data.
...
- `JHcurve` = true (nonlinear behaviour)
   ...
   Value `files`: list of filenames with the *H-J* curves of the PM materials
   Value `tref`: reference temperatures (in celsius degrees) corresponding to the *H-J* curves described in the previous files
   Value `jvt`: coefficients for linear rescaling values of residual polarization $J_r$, as
   ...
   Values `hvt1,hvt2`: linear and quadratic coefficients for rescaling values of coercive field $H_c$


   $H_c=H_{co}\,(1+a\,(T-T_{ref})+b\,(T-T_{ref})^2)$
   
   where
   
   $H_{co}$ is the coercive field of the *H-J* curve at the reference temperature $T_{ref}$ as given in value `tref`

   $a$ is the value of the coefficient given in value `hvt1`.

   $b$ is the value of the coefficient given in value `hvt2`.


- `JHcurve` = false (linear behaviour)

   Value `MuR`: relative permeability $\mu_R$ of the PM materials, as
   
   $B = \mu_o H + J = \mu_o H + J_r + \mu_R \mu_o H$
   
   Value `Jresidua`: residual polarization $J_r$ (in tesla) of the PM materials
...
- `type` = 'MC' stochastic simulation, that is Montecarlo simulation using the variability of input data provided in the tavle `[montecarlo]`
